...
def get_virtual_memory_windows():
    """Obtiene información de memoria virtual en Windows"""
    try:
        import ctypes
...
def get_swap_or_virtual():
    ...
    if IS_WINDOWS:
        mem_virtual = get_virtual_memory_windows()
        if mem_virtual:
            return {
                'name': 'MEM VIRTUAL',
            ...
            # Fallback: usar pagefile.sys
            swap = psutil.swap_memory()
            return {
                'name': 'MEM VIRTUAL',
                'total': swap.total if swap.total > 0 else 16 * 1024**3,
                'used': swap.used,
                'free': swap.free,
...
def get_cpu_temp_windows():
    """Obtiene temperatura CPU en Windows"""
    try:
        if HAS_WMI:
            w = wmi.WMI(namespace="root\\OpenHardwareMonitor")
            for sensor in w.Sensor(SensorType='Temperature'):
...
def get_cpu_temp_linux():
    """Obtiene temperatura CPU en Linux"""
    temps = []
    try:
...
# ─── TEMPERATURAS DISCOS ────────────────────────────────────────────────────
def get_disk_temps():
    """Obtiene temperaturas de discos"""
    disk_temps = {}
    ...
    if not disk_temps:
        # Simular temperatura basada en actividad
        try:
...
def render(static: dict, cols: int, first_render: bool = False):
    ...
    # ─── RECURSOS (RAM + SWAP/MEMORIA VIRTUAL) ─────────────────────────────
    ...
    # Usar función unificada para SWAP/Memoria Virtual
...
def main():
    ...
    if not IS_WINDOWS:
        ...
        # Windows: manejo de redimensionamiento
        try:
            import msvcrt
            first = True
            last_cols = 0
